...
Le développement de Prysma repose sur une rigueur architecturale stricte. L'objectif principal est de maintenir un compilateur robuste, déterministe et performant. Toute contribution doit s'aligner sur ces standards d'ingénierie. 
...
Le cœur du compilateur est écrit en C++17. Les règles suivantes sont non négociables :

* **Séparation des préoccupations :** Respectez le découplage strict entre le Frontend (Lexer/Parseur) et le Backend (LLVM). 
...
* **Gestion de la mémoire :** L'allocation interne du compilateur est gérée par notre architecture Arena (Bump Pointer Allocator). L'utilisation de pointeurs intelligents standards (`std::shared_ptr`) est prohibée dans le flux critique de l'AST pour des raisons de performance. Aucune fuite mémoire n'est tolérée.
* **Design Patterns :** Les modifications de la génération de code doivent respecter le contrat du patron **Visiteur**.
...
4. **Validation obligatoire :** Vous devez exécuter l'intégralité de la suite de tests (unitaires et de charge) via l'orchestrateur avant toute soumission.
   ```bash
   python3 build.py