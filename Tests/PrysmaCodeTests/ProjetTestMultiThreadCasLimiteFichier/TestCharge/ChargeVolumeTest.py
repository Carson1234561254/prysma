...
with open(f"{DOSSIER}/main.p", "w") as main_file:
   ...
   # on créer une fonction qui appelle la dernière fonction de chaque fichier 
    appels = " + ".join([f"call func_{i}_{FONCTIONS_PAR_FICHIER}()" for i in range(1, NB_FICHIERS + 1)])
    main_file.write(f"    dec int64 resultat = {appels};\n")
