...
def ecrire_constructeurs(fh, nom_classe, proprietes):
    ...
    if proprietes:
        params, inits = [], []
        items = list(proprietes.items())
        
        # Déterminer les indices finaux consécutifs qui sont des pointeurs 
        trailing_default_start = len(items)
        for i in range(len(items) - 1, -1, -1):
            prop_type = items[i][1]
            if valeur_defaut_parametre(prop_type) is not None:
                trailing_default_start = i
            else:
                break
        
        for idx, (prop_nom, prop_type) in enumerate(items):
            type_param, format_init = analyser_parametre(prop_type)
            defaut = valeur_defaut_parametre(prop_type)
            if defaut is not None and idx >= trailing_default_start:
                params.append(f"{type_param} {prop_nom} = {defaut}")
            else:
